...
const GraphQLJson = require("./scalarsJson");
          ...
          if (rules.hasOwnProperty("model")) {
            fields[key] = {
              ...
              resolve: function(object) {
                const criteria = {};
                criteria[collection.primaryKey] =
                  object[key][
                    self.defaults.collections[rules.model].primaryKey
                  ];

                return self.defaults.queryFields[
                  rules.model.toLowerCase()
            ...
            fields[key] = {
              type: new GraphQL.GraphQLList(
                self.defaults.types[_.capitalize(rules.collection)]
              ),
              resolve: function(object) {
                const criteria = {};
                criteria[rules.via.toLowerCase()] =
                  object[collection.primaryKey];

                return self.defaults.queryFields[
                  rules.collection.toLowerCase() + "s"
                ].resolve(object, criteria);
              }
            };
          } else {
            fields[key] = {
              type:
                // If the attribute is required or it is "id"
                rules.required || key === "id"
                  ? new GraphQL.GraphQLNonNull(convertToGraphQLType(rules))
                  : convertToGraphQLType(rules)
            };
          }
        });
  ...
  buildQueryFields: function(collection) {
    ...
    fields[collectionIdentity.toLowerCase() + "s"] = {
      ...
      resolve: function(object, criteria, parent) {
        const filters = _.omit(criteria, function(value) {
    ...
    _.forEach(_.omit(fields, collectionIdentity.toLowerCase()), function(
      field
    ) {
      ...
      resolve: function resolve(object, criteria) {
        ...
        return Promise.all(arrayOfPromises)
          .then(function(results) {
            ...
            object.type =
              self.defaults.queryFields[
                _.keys(self.defaults.queryFields)[typeIndex]
              ].type;
